...
app.get(
  "/all_session/:id/:sports_name/report",
...
app.get(
  "/add_sports",
  async (request, response) => {
    response.render("New_Sports_add", {
      Title: "create sports",
...
app.post("/admins", async (request, response) => {
  ...
  if (request.body.name != "balamurugan@gmail.com") {
    request.flash("error", "Email is rong!");
    return response.redirect("/login");
  }
  if (request.body.password != "admin@123") {
    request.flash("error", "are you admin?");
    return response.redirect("/login");
  } else {
...
app.get(
  ...
  async (request, response) => {
    response.render("session_Update", {
      title: localStorage.getItem("current_sports_name"),
